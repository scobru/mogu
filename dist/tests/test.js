...
const RESTORE_DIR = path_1.default.join(process.cwd(), 'test-restored');
const STORAGE_DIR = path_1.default.join(process.cwd(), 'test-storage');
...
async function run() {
    try {
        ...
        await fs_extra_1.default.ensureDir(STORAGE_DIR);
        ...
        await fs_extra_1.default.remove(STORAGE_DIR).catch(() => { });
    }
}
...
async function testCacheSystem() {
    ...
    if (avgTime2 > avgTime1 * 0.95) {
        throw new Error(`Cache restore is not significantly faster\n` +
    ...
    if (modBackup1.hash === modBackup2.hash) {
        throw new Error('Backups should be different after file modification\n' +
...
async function testCompare() {
    const mogu = new mogu_1.Mogu(baseConfig);
    try {
        // Test 1: Compare identical directories
        console.log("\nTest 1: Compare identical directories");
        const testDir = path_1.default.join(TEST_DIR, 'compare-test');
        ...
        const directories = new Set(Object.keys(initialFiles)
            .map(filePath => path_1.default.dirname(filePath))
            .filter(dir => dir !== '.'));
        for (const dir of directories) {
            const fullDirPath = path_1.default.join(testDir, dir);
            console.log(`Creating directory: ${fullDirPath}`);
            await fs_extra_1.default.ensureDir(fullDirPath);
        }
        // Create all files
        for (const [filePath, content] of Object.entries(initialFiles)) {
            const fullPath = path_1.default.join(testDir, filePath);
            console.log(`Creating file: ${fullPath}`);
            await fs_extra_1.default.writeFile(fullPath, content);
        }
        // Verify original directory structure
        const originalFiles = await fs_extra_1.default.readdir(testDir, { recursive: true, withFileTypes: true });
        ...
        while (retries > 0) {
            ...
            const tempRestoreDir = path_1.default.join(TEST_DIR, 'temp-restore');
            await fs_extra_1.default.emptyDir(tempRestoreDir);
            try {
                // Create subdirectories in restore path
                for (const dir of directories) {
                    const restoreDirPath = path_1.default.join(tempRestoreDir, dir);
                    console.log(`Creating restore directory: ${restoreDirPath}`);
                    await fs_extra_1.default.ensureDir(restoreDirPath);
                }
                ...
                const restoredFiles = await fs_extra_1.default.readdir(tempRestoreDir, { recursive: true, withFileTypes: true });
                ...
                for (const [filePath, expectedContent] of Object.entries(initialFiles)) {
                    const restoredPath = path_1.default.join(tempRestoreDir, filePath);
                    console.log(`\nVerifying file: ${filePath}`);
                    console.log(`Full path: ${restoredPath}`);
                    if (!await fs_extra_1.default.pathExists(restoredPath)) {
                        console.log(`Missing file: ${filePath}`);
                        const dirPath = path_1.default.dirname(restoredPath);
                        const dirExists = await fs_extra_1.default.pathExists(dirPath);
                        console.log(`Directory ${dirPath} exists: ${dirExists}`);
                        if (dirExists) {
                            const dirContents = await fs_extra_1.default.readdir(dirPath);
                            console.log(`Contents of directory ${dirPath}:`, dirContents);
                        }
                        allFilesRestored = false;
                        break;
                    }
                    const restoredContent = await fs_extra_1.default.readFile(restoredPath, 'utf8');
                ...
                if (allFilesRestored) {
                    ...
                    await fs_extra_1.default.remove(tempRestoreDir);
                    ...
                    const allFiles = await fs_extra_1.default.readdir(tempRestoreDir, { recursive: true, withFileTypes: true });
                    console.log("\nFiles present in restore directory:");
                    allFiles.forEach(file => {
                        const filePath = file.path ? path_1.default.join(file.path, file.name) : file.name;
        ...
        await new Promise(resolve => setTimeout(resolve, 2000));
        // Modify some files
        await fs_extra_1.default.writeFile(path_1.default.join(testDir, 'file1.txt'), 'Modified content');
        await fs_extra_1.default.writeFile(path_1.default.join(testDir, 'newfile.txt'), 'New content');
        await fs_extra_1.default.remove(path_1.default.join(testDir, 'file2.json'));
        await fs_extra_1.default.remove(path_1.default.join(testDir, 'subdir/file3.txt'));
        // Verify that modifications were applied
        const modifiedContent = await fs_extra_1.default.readFile(path_1.default.join(testDir, 'file1.txt'), 'utf8');
        ...
        if (emptyComparison.totalChanges.deleted !== expectedDeletedFiles) {
            throw new Error(`Wrong number of deleted files.\n` +
...
async function testVersioning() {
    ...
    const version1Dir = path_1.default.join(TEST_DIR, 'version1');
    const version2Dir = path_1.default.join(TEST_DIR, 'version2');
    const version3Dir = path_1.default.join(TEST_DIR, 'version3');
    await fs_extra_1.default.ensureDir(version1Dir);
    await fs_extra_1.default.ensureDir(version2Dir);
    await fs_extra_1.default.ensureDir(version3Dir);
    ...
    for (const [name, content] of Object.entries(initialFiles)) {
        await fs_extra_1.default.writeFile(path_1.default.join(version1Dir, name), content);
    }
    ...
    for (const [name, content] of Object.entries(version2Files)) {
        await fs_extra_1.default.writeFile(path_1.default.join(version2Dir, name), content);
    }
    ...
    for (const [name, content] of Object.entries(version3Files)) {
        await fs_extra_1.default.writeFile(path_1.default.join(version3Dir, name), content);
    }
    ...
    const v1RestoreDir = path_1.default.join(RESTORE_DIR, 'v1');
    const v2RestoreDir = path_1.default.join(RESTORE_DIR, 'v2');
    const v3RestoreDir = path_1.default.join(RESTORE_DIR, 'v3');
    await fs_extra_1.default.ensureDir(v1RestoreDir);
    await fs_extra_1.default.ensureDir(v2RestoreDir);
    await fs_extra_1.default.ensureDir(v3RestoreDir);
    ...
    for (const [name, expectedContent] of Object.entries(initialFiles)) {
        const content = await fs_extra_1.default.readFile(path_1.default.join(v1RestoreDir, name), 'utf8');
        if (content !== expectedContent) {
    ...
    for (const [name, expectedContent] of Object.entries(version2Files)) {
        const content = await fs_extra_1.default.readFile(path_1.default.join(v2RestoreDir, name), 'utf8');
        if (content !== expectedContent) {
    ...
    for (const [name, expectedContent] of Object.entries(version3Files)) {
        const content = await fs_extra_1.default.readFile(path_1.default.join(v3RestoreDir, name), 'utf8');
        if (content !== expectedContent) {
            throw new Error(`Content mismatch in version 3 for ${name}`);
        }
    }
    // Verify file2.txt doesn't exist in version 3
    const file2Exists = await fs_extra_1.default.pathExists(path_1.default.join(v3RestoreDir, 'file2.txt'));
    if (file2Exists) {
        throw new Error('file2.txt should not exist in version 3');
    }
    // Cleanup
    await fs_extra_1.default.remove(version1Dir);
    await fs_extra_1.default.remove(version2Dir);
    await fs_extra_1.default.remove(version3Dir);
    await fs_extra_1.default.remove(v1RestoreDir);
    await fs_extra_1.default.remove(v2RestoreDir);
    await fs_extra_1.default.remove(v3RestoreDir);
    console.log("Versioning tests completed successfully!");
}
async function testDelete() {
    const mogu = new mogu_1.Mogu(baseConfig);
    // Create test directory and file
    const testDir = path_1.default.join(TEST_DIR, 'delete-test');
    await fs_extra_1.default.ensureDir(testDir);
    await fs_extra_1.default.writeFile(path_1.default.join(testDir, 'test.txt'), 'Test content');
    // Create backup
    ...
    const restoreDir = path_1.default.join(RESTORE_DIR, 'delete-test');
    await fs_extra_1.default.ensureDir(restoreDir);
    await mogu.restore(backup.hash, restoreDir);
    const restoredContent = await fs_extra_1.default.readFile(path_1.default.join(restoreDir, 'test.txt'), 'utf8');
